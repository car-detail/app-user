...
class BookingDataManager {
  ...
  getServiceDetails(BuildContext context, String id) async {
    try {
      ...
      print("🔍 [Booking] Trying vendor services endpoint first for ID: $id");
      
      try {
        var response = await apiFuntions.getdatauser(context, "v1/services/vendor/$id");
        if (response.statusCode == 200) {
          print("✅ [Booking] Vendor services endpoint succeeded");
          return response;
        } else if (response.statusCode == 400) {
          print("❌ [Booking] Vendor services returned 400, trying service-details endpoint");
          ...
          print("❌ [Booking] Vendor services returned ${response.statusCode}, trying service-details endpoint");
          // For other errors, try service-details as fallback
          return await apiFuntions.getdatauser(context, "${Constant.serviceDetails}$id");
        }
      } catch (e) {
        print("❌ [Booking] Vendor services endpoint failed: $e, trying service-details endpoint");
        // If vendor services fails, try service-details
        return await apiFuntions.getdatauser(context, "${Constant.serviceDetails}$id");
      }
    } catch (e) {
      print("❌ [Booking] Error in getServiceDetails API call: $e");
